...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8008/produits/api/categorie/all').then((response) => {
      setCategories(response.data);
      
    });

    axios.get(`http://localhost:8008/produits/api/getbyid/${id}`)
      .then(response => {
        ...
        setCode(code);
        setNewCode(code);
        axios.get(`http://localhost:8008/produits/api/categorie/getbyid/${categorie.idCategorie}`)
      .then(response => {
...
const validateForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
      ...
      try {
        console.log(id);
      await axios.put(`http://localhost:8008/produits/api/updateproduit/${id}`, data);
      navigate('/Product');
    } catch (error) {
      console.error(error);
